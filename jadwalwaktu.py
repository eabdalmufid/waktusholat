# JANGAN LUPA SHOLAT YA BRO/MBAK
# SHOLAT ITU WAJIB!
# MARI KITA SHOLAT YA BRO/MBAK
# SCRIPT BY ABDUL MUFID (Jangan Dihapus)
...
def gettime():
      ...
      try:
                        ...
                        print(lg+'\nAstaghfirullah..\nUkhty/akhty lupa ngidupin jaringannya'+x)
                        input(lg+'\nEnterin aja')
                        menu()
...
{lw}AUTHOR   : Abdul Mufid{lg}
{lw}GITHUB   : github.com/eabdalmufid{lg}
{lw}WHATSAPP : 19047502362{lg}
{lw}TEMA     : JADWAL SHOLAT{lg}

              {lw}Jangan Lupa Sholat Dan Jangan Ditinggalkan {lg}
                    {lw}Selamat Menunaikan Ibadah Sholat{lg}
...
def menu():
      banner()
      print(f'''
{lg}1.{lw} Aktifkan lagi
{lg}2.{lw} Ganti kota
{lg}3.{lw} Update jaringan
{lg}4.{lw} Tentang
{lg}0.{lw} Keluar''')
...
def update():
      banner()
      print(lr+'Jangan di cancell ya ukhty/akhty.. biar nggak error :*')
      print(lg+'Cek jaringan..')
      try:
            get('https://github.com/eabdalmufid/waktusholat')
      except requests.exceptions.ConnectionError:
            print(lg+'Astaghfirullah.. Ukhty/Akhty lupa ngidupin jaringannya')
            exit()
      print(lg+'Mengupdate..\nLama tidaknya tergantung jaringan, sabarr :)')
      
      os.system('cd .. && rm -rf sholat')     
      sp.call(['cd .. && git clone https://github.com/eabdalmufid/waktusholat'],shell=True, stdout=sp.DEVNULL,stderr=sp.STDOUT)
      print(lg+'Selesai mengupdate')
      print(lg+'Memulai ulang..')
      sleep(2)
      os.system('cd waktusholat && python jadwalwaktu.py')    
def tentang():
      banner()
      print(f'''
         
{lg}Nama        : {lw}Jadwal Sholat
{lg}Versi       : {lw}3.8 (update: 28 November 2022, 06:40AM)
{lg}Tanggal     : {lw}28 November 2022, 06:40AM
{lg}Author      : {lw}Abdul Mufid
{lg}Tujuan      : {lw}Mengingatkan kita pada waktu sholat
{lg}Terimakasih : {lw}Allah SWT dan semua umat Muslim seplanet bumi
{lg}NB          : {lw}Manusia nggak ada yang sempurna,
              sama kaya tool ini.
              Silahkan laporkan kritik atau saran
              ke :  - https://www.github.com/eabdalmufid
                    - https://www.instagram.com/eabdalmufid_''')
...
def exit():
      print(lg+'_'*71)
      print('makasih ukhty/akhty nanti aktifkan lagi ya😘'+x)
